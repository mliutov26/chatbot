...
if (!empty($botSettings['SETTINGS']['MANAGERS'])) {
	foreach ($botSettings['SETTINGS']['MANAGERS'] as $keyManager => $manager) {
		$arrayManager = array(0 => $keyManager);
	}
} else {
	$arrayManager = array(0 => 1);
}
 
$userAdd = restCommand('imbot.chat.user.add', Array(
...
$result = restCommand('imbot.message.add', Array(
	...
	"MESSAGE" => 'Оператор свяжется с вами в ближайшее время.',
), $_REQUEST["auth"]);
...
if (array_key_exists('error', $resWater) && !empty($resWater['error'])) {
	...
	if(empty($resWater['result'])) {
		...
		if ($chatType['0'] == 'livechat' || $chatType['0'] == 'network') {
			$result = restCommand('imbot.message.add', Array(
				"DIALOG_ID" => $_REQUEST['data']['PARAMS']['DIALOG_ID'],
				"MESSAGE" => 'В каталоге воды еще нет товаров[br][br][send=0]Связаться с оператором[/send]',
			), $_REQUEST["auth"]);
		} else {
			$result = restCommand('imbot.message.add', Array(
				"DIALOG_ID" => $_REQUEST['data']['PARAMS']['DIALOG_ID'],
		...
		if ($chatType['0'] == 'livechat' || $chatType['0'] == 'network') {
			...
			if ($botSettings['SETTINGS']['REFUND_MESS']) {
				$messCommand = '[send=new_order_refund]Продолжить оформление заказа[/send]';
			} elseif (!empty($resAccessories['result'])) {
				$messCommand = '[send=new_order_accessories]Продолжить оформление заказа[/send]';
			} else {
				$messCommand = '[send=see_delivery]Продолжить оформление заказа[/send]';
			}
			$messCommand .= '[br][send=0]Связаться с оператором[/send]';
			
		} else {
			
			$messCommand = '1: Продолжить оформление заказа[br]';
