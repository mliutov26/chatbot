...
if($event == 'ONIMCOMMANDADD') {
	if($command['COMMAND'] !== 'exit_state' && $command['COMMAND'] !== 'reset_settings') {
		$result = restCommand('imbot.command.answer', Array(
			...
			'MESSAGE' => 'Бот находится в режиме добавления текста благодарности за оформление заказа. Введите сообщение или [send=/exit_state]выйдите из режима[/send]',
			'KEYBOARD' => $keyboard,
		), $arAuth);
		exit;
	}
	...
	$resultThanks = restCommand('imbot.message.add', Array(
		'DIALOG_ID' => $dialogId,
		'MESSAGE' => 'Текст благодарности за оформление заказа добавлен. [send=/thanks]Посмотреть[/send]',
		'KEYBOARD' => $keyboard,
	), $arAuth);
	exit;
}
