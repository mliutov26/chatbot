...
if($event == 'ONIMCOMMANDADD') {
	if($command['COMMAND'] !== 'exit_state' && $command['COMMAND'] !== 'reset_settings') {
		$result = restCommand('imbot.command.answer', Array(
			...
			'MESSAGE' => 'Бот находится в режиме добавления приветственного сообщения. Введите сообщение или [send=/exit_state]выйдите из режима[/send]',
			'KEYBOARD' => $keyboard,
		), $arAuth);
		exit;
	}
	...
	$resultHello = restCommand('imbot.message.add', Array(
		'DIALOG_ID' => $dialogId,
		'MESSAGE' => 'Приветственное сообщение добавлено. [send=/hello]Проверить[/send]',
		'KEYBOARD' => $keyboard,
	), $arAuth);
	exit;
}
